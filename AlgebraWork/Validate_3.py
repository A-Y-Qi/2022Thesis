...
for r in range (0,N):
    function[r]=1/sqrt(N)*(2*sin(2*pi*(1/(2*N)+r/N)))**power
...
for s in range(0,N):
    ...
    for p in range (-N,N):
        ...
        if nv_2 in compare:
            nv=nv_2/2
            add=add+(math.factorial(power)/(math.factorial(nv)*math.factorial(power-nv)))*exp(-2*pi*1j*1/(2*N)*(nv_2-power))*((-1)**(nv))
    result[s]=add*(-1)**(power)*((1j)**(power))
