...
for t in range(0,nmax):
    for q in range(0, m):
        ...
        ur_hat=u_hat[r]
        up_hat=u_hat[p]
        unew_hat[q]=newtimestep(uq_hat, ur_hat, up_hat, F, gamma, dt, plus, minus)
...
density=array(angle_list(64,udata1,N))
...
def animate(i):
    ...
    ax1.set_xlabel('x')
    ax1.set_ylabel('y') 
    plt.suptitle("Simulation using Euler when N="+str(N)+" m="+str(m)+" dt="+str(dt))
    if k > nmax-1:
        k=0

    
anim = animation.FuncAnimation(fig,animate)
#anim.suptitle("Simulation of convolution using AB/BDI2 with sigma="+str(sigma)+" dt="+str(dt)+"N="+str(N))
#ax1.view_init(90, 90)
plt.show()    
