...
def w_func_gaussian_2(N, sigma,kk,a_or_r):
    #define theta and phi
    phi_prime = (2*pi/N)*arange(-N/2.0,N/2.0)
    phi = (2*pi/N)*arange(-N/2.0,N/2.0)
    #define Sx and Sy
            ...
            for k in range (0,N):
                for l in range (0,N):
                    ...
                    if diff<-pi:
                        diff=2*pi+diff
                    v=kk*(diff)
                    phi_diff_temp=phi_angle-phi_prime_temp
                    if phi_diff_temp>pi:
                        phi_diff_temp=-2*pi+phi_diff_temp
                    if phi_diff_temp<-pi:
                        phi_diff_temp=2*pi+phi_diff_temp
                    theta=phi_diff_temp-v
                    #if theta==0:
                        #print(i,j,k,l)
                    if theta>pi:
                        theta=-2*pi+theta
                    if theta<-pi:
                        theta=2*pi+theta
                    temp_w=(1/(sqrt(pi)*sigma))*exp(-((theta)/sigma)**2)
                    w_func_4D[i,j,k,l]=temp_w
                    phi_prime_temp=0
                    phi_diff_temp=0
                    diff=0
                    temp_w=0
